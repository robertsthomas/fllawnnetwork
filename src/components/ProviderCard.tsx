...
export default function ProviderCard({ provider }: ProviderCardProps) {
  ...
  return (
    <Card className={`h-full overflow-hidden ${featured ? 'border-2 border-accent-500' : ''}`}>
      <Link href={`/providers/${provider._id.toString()}`} prefetch={true} onClick={handleProfileClick}>
        ...
        <div className="flex flex-col h-[calc(100%-12rem)]">
          <CardContent className="p-4 pt-8 flex-grow">
          ...
          <CardFooter className="p-4 pt-0 mt-auto">
            <Button asChild variant="default" className="w-full" onClick={handleProfileClick}>
              <Link href={`/providers/${provider._id.toString()}`} prefetch={true}>View Profile</Link>
            </Button>
          </CardFooter>
        </div>
      </Link>
